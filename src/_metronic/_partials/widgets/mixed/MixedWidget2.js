...
export function MixedWidget2({ className }) {
  ...
  useEffect(() => {
    ...
    chart.data = [
      {
        facultad: "Pre-registro Medicina",
        aspirantes: 1760
      }, 
      {
        facultad: "Aceptados en medicina",
        aspirantes: 204
      },
      {
        facultad: "Indigenas",
      ...
      {
        facultad: "Trabajadores/Hijos",
        aspirantes: 12
      },
    ];

    let series = chart.series.push(new am4charts.PieSeries3D());
    series.dataFields.value = "aspirantes";
    series.dataFields.category = "facultad";
  ...
  return (
    <div className={`card card-custom bg-gray-100 ${className}`} style={{ height: 'auto' }} >
      {/* Header */}
      <div className="card-header border-0 bg-danger py-5">
        <h3 className="card-title font-weight-bolder text-white">Aspirantes</h3>
...
function getChartOptions(layoutProps) {
  ...
  const options = {
    series: [
      {
        name: "Total",
        data: [30, 45, 32, 70, 40, 40, 40, 33, 34, 42, 44, 50]
      }
    ],
    chart: {
      type: "pie3d",
